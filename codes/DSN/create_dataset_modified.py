...
def domain_distance_map_handler(fake_img, D_out, convnet, fs_type):
    if fs_type.lower() == 'gau' or fs_type == 'avg_pool':
        ddm_shape = (fake_img.shape[0], 1,
                     fake_img.shape[2], fake_img.shape[3])
    elif fs_type.lower() == 'wavelet':
        ddm_shape = (
            fake_img.shape[0], 1, fake_img.shape[2] // 2, fake_img.shape[3] // 2)
    else:
        raise NotImplementedError(
            'Frequency Separation [{:s}] not recognized'.format(fs_type))
    ddm = torch.zeros(ddm_shape)
    currentLayer_h, currentLayer_w = receptive_cal(
        ddm.shape[2], convnet), receptive_cal(ddm.shape[3], convnet)
...
parser.add_argument('--generator', default='DeResnet',
                    type=str, help='Generator model to use')
parser.add_argument('--num_res_blocks', default=8,
                    type=int, help='number of ResNet blocks')
parser.add_argument('--discriminator', default='FSD',
                    type=str, help='Discriminator model to use')
...
parser.add_argument('--cat_or_sum', default='cat',
                    type=str, help='set wavelet bands type')
parser.add_argument('--norm_layer', default='Instance',
                    type=str, help='set type of discriminator norm layer')
parser.add_argument('--artifacts', default='tdsr', type=str,
                    help='selecting different artifacts type')
parser.add_argument('--name', default='0603_DSN_LRs', type=str,
                    help='additional string added to folder path')
parser.add_argument('--dataset', default='aim2019', type=str,
...
parser.add_argument('--upscale_factor', default=4, type=int,
                    choices=[4, 1, 2], help='super resolution upscale factor')
...
elif opt.dataset == 'facesr':
    input_source_dir = PATHS['facesr']['tdsr']['source']
    input_target_dir = PATHS['facesr']['tdsr']['target']
    source_files = [os.path.join(input_source_dir, x) for x in os.listdir(
        input_source_dir) if utils.is_image_file(x)]
    target_files = [os.path.join(input_target_dir, x) for x in os.listdir(
        input_target_dir) if utils.is_image_file(x)]
...
if opt.checkpoint is not None:
    ...
    print('Copying {} to {}'.format(opt.checkpoint,
          os.path.join(tdsr_lr_dir, opt.name+'.tar')))
...
with torch.no_grad():
    for _, files in enumerate(tqdm(list(batching(target_files, 3)), desc='Generating images from target')):
        # load HR image
        tf_img_list = []
        for file in files:
            input_img = Image.open(file).resize((512, 512))
            input_img = TF.to_tensor(input_img).unsqueeze(0)
            # print(input_img.shape)
            tf_img_list.append(input_img)

        input_tf = torch.cat(tf_img_list)
        # Apply model to input_img
        if torch.cuda.is_available():
            #input_img = input_img.unsqueeze(0).cuda()
            input_tf = input_tf.cuda()
        fake_imgs = model_g(input_tf)

        for it_img in range(fake_imgs.size()[0]):
            fake_img = fake_imgs[it_img, :, :, :].unsqueeze(0)
            D_out = model_d(fake_img).cpu().detach().numpy()
            ddm = domain_distance_map_handler(
                fake_img, D_out, convnet, opt.filter)
            # Save input_noisy_img as HR image for TDSR
            fake_img = fake_img.squeeze(0).cpu()
            path = os.path.join(
                tdsr_lr_img_dir, os.path.basename(files[it_img]).split('.')[0]+'.png')
            TF.to_pil_image(fake_img).save(path, 'PNG')
            np.save(os.path.join(tdsr_lr_ddm_t_dir,
                    os.path.basename(files[it_img]).split('.')[0]), ddm)
...
if opt.including_source_ddm:
    with torch.no_grad():
        for file in tqdm(source_files, desc='Generating images from source files'):
            ...
            ddm = domain_distance_map_handler(
                input_img, D_out, convnet, opt.filter)
            np.save(os.path.join(tdsr_lr_ddm_s_dir,
                    os.path.basename(file).split('.')[0]), ddm)
