...
parser.add_argument(
    "--crop_size", default=256, type=int, help="training images crop size"
)
parser.add_argument(
    "--crop_size_val", default=256, type=int, help="validation images crop size"
)
...
parser.add_argument(
    "--num_epochs", default=400, type=int, help="total train epoch number"
)
parser.add_argument(
    "--num_decay_epochs",
    default=150,
    type=int,
    help="number of epochs during which lr is decayed",
)
...
parser.add_argument(
    ...
    default=0.5,
    type=float,
    help="beta_1 for adam optimizer of gen and disc",
)
...
parser.add_argument(
    ...
    default=5,
    type=int,
    help="interval for saving validation images",
)
parser.add_argument(
    "--save_model_interval", default=5, type=int, help="interval for saving the model"
)
parser.add_argument(
    "--artifacts", default="tdsr", type=str, help="selecting different artifacts type"
)
parser.add_argument(
...
parser.add_argument(
    ...
    dest="flips",
    action="store_true",
    help="if activated train images are randomly flipped",
)
parser.add_argument(
    "--rotations",
    dest="rotations",
...
parser.add_argument(
    "--num_res_blocks", default=8, type=int, help="number of ResNet blocks"
)
parser.add_argument(
    "--ragan",
    dest="ragan",
...
parser.add_argument(
    ...
    default=5,
    type=int,
    help="kernel size used in transformation for discriminators",
)
parser.add_argument(
    "--no_per_loss",
    dest="use_per_loss",
...
parser.add_argument(
    ...
    default="LPIPS",
    type=str,
    help="selecting different Perceptual loss type",
)
parser.add_argument(
    "--disc_freq",
    default=1,
    type=int,
    help="number of steps until a discriminator updated is made",
)
parser.add_argument(
    "--gen_freq",
    default=1,
    type=int,
    help="number of steps until a generator updated is made",
)
...
parser.add_argument(
    "--w_per", default=0.01, type=float, help="weight of perceptual loss"
)
parser.add_argument(
    "--checkpoint", default=None, type=str, help="checkpoint model to start from"
)
parser.add_argument(
    "--save_path", default=None, type=str, help="additional folder for saving the data"
)
parser.add_argument(
    "--generator",
    default="DeResnet",
...
parser.add_argument(
    "--filter", default="gau", type=str, help="set filter in (gau, avg_pool, wavelet)"
)
parser.add_argument(
    "--cat_or_sum", default="cat", type=str, help="set wavelet bands type in (cat, sum)"
)
parser.add_argument(
    "--norm_layer",
    default="Instance",
...
def main():
    ...
    if opt.dataset == "aim2019":
        train_set = loader.Train_Deresnet_Dataset(
            PATHS["aim2019"][opt.artifacts]["source"],
            PATHS["aim2019"][opt.artifacts]["target"],
            cropped=True,
            **vars(opt)
        )
        ...
        val_set = loader.Val_Deresnet_Dataset(
            PATHS["aim2019"][opt.artifacts]["valid_hr"],
            lr_dir=PATHS["aim2019"][opt.artifacts]["valid_lr"],
            **vars(opt)
        )
        ...
        train_set = loader.Train_Deresnet_Dataset(
            PATHS["ntire2020"][opt.artifacts]["source"],
            PATHS["ntire2020"][opt.artifacts]["target"],
            cropped=True,
            **vars(opt)
        )
        train_loader = DataLoader(
        ...
        val_set = loader.Val_Deresnet_Dataset(
            PATHS["ntire2020"][opt.artifacts]["valid_hr"],
            lr_dir=PATHS["ntire2020"][opt.artifacts]["valid_lr"],
            **vars(opt)
        )
        val_loader = DataLoader(
            dataset=val_set, num_workers=1, batch_size=1, shuffle=False
        ...
        train_set = loader.Train_Deresnet_Dataset(
            PATHS["realsr"][opt.artifacts]["source"],
            PATHS["realsr"][opt.artifacts]["target"],
            cropped=True,
            **vars(opt)
        )
        train_loader = DataLoader(
        ...
        val_set = loader.Val_Deresnet_Dataset(
            PATHS["realsr"][opt.artifacts]["valid_hr"],
            lr_dir=PATHS["realsr"][opt.artifacts]["valid_lr"],
            **vars(opt)
        )
        val_loader = DataLoader(
            dataset=val_set, num_workers=1, batch_size=1, shuffle=False
        ...
        train_set = loader.Train_Deresnet_Dataset(
            PATHS["camerasr"][opt.artifacts]["source"],
            PATHS["aim2019"][opt.artifacts]["target"],
            cropped=True,
            **vars(opt)
        )
        ...
        val_set = loader.Val_Deresnet_Dataset(
            PATHS["camerasr"][opt.artifacts]["valid_hr"],
            lr_dir=PATHS["camerasr"][opt.artifacts]["valid_lr"],
            **vars(opt)
        )
    elif opt.dataset == "face_sr":
        train_set = loader.Train_Deresnet_Dataset(
            PATHS["face_sr"][opt.artifacts]["source"],
            PATHS["face_sr"][opt.artifacts]["target"],
            cropped=True,
            **vars(opt)
        )
        train_loader = DataLoader(
        ...
        val_set = loader.Val_Deresnet_Dataset(
            PATHS["face_sr"][opt.artifacts]["valid_hr"],
            lr_dir=PATHS["face_sr"][opt.artifacts]["valid_lr"],
            **vars(opt)
        )
        val_loader = DataLoader(
            dataset=val_set, num_workers=1, batch_size=1, shuffle=False
        ...
        train_set = loader.TrainDataset(
            PATHS[opt.dataset][opt.artifacts]["hr"]["train"], cropped=True, **vars(opt)
        )
        train_loader = DataLoader(
            dataset=train_set,
            num_workers=opt.num_workers,
        ...
        val_set = loader.ValDataset(
            PATHS[opt.dataset][opt.artifacts]["hr"]["valid"],
    ...
    if opt.generator.lower() == "dsgan":
        ...
        model_g = model.De_resnet(
            n_res_blocks=opt.num_res_blocks, scale=opt.upscale_factor
        )
    else:
        raise NotImplementedError(
            "Generator model [{:s}] not recognized".format(opt.generator)
        )
    ...
    model_d = model.Discriminator(
        ...
        D_arch=opt.discriminator,
        norm_layer=opt.norm_layer,
        filter_type=opt.filter,
        cs=opt.cat_or_sum,
    )
    print(
    ...
    filter_low_module = model.FilterLow(
        kernel_size=opt.kernel_size, gaussian=opt.filter == "gau", include_pad=False
    )
    filter_high_module = model.FilterHigh(
        kernel_size=opt.kernel_size, gaussian=opt.filter == "gau", include_pad=False
    )
    ...
    optimizer_g = optim.Adam(
        model_g.parameters(), lr=opt.learning_rate, betas=[opt.adam_beta_1, 0.999]
    )
    optimizer_d = optim.Adam(
        model_d.parameters(), lr=opt.learning_rate, betas=[opt.adam_beta_1, 0.999]
    )
    start_decay = opt.num_epochs - opt.num_decay_epochs
    scheduler_rule = (
        lambda e: 1.0
        if e < start_decay
        else 1.0 - max(0.0, float(e - start_decay) / opt.num_decay_epochs)
    )
    ...
    for epoch in range(start_epoch, opt.num_epochs + 1):
        ...
        for input_img, bicubic_img, disc_img in train_bar:
            ...
            if iteration % opt.disc_freq == 0:
                ...
                if opt.wgan:
                    ...
                    gradient = torch.autograd.grad(
                        gp_tex.mean(), sample, create_graph=True
                    )[0]
                ...
                model_d.zero_grad()
                d_tex_loss = loss.discriminator_loss(
                    real_tex, fake_tex, wasserstein=opt.wgan, grad_penalties=grad_pen
                )
                ...
                if opt.saving:
                    ...
                    if opt.wgan:
                        writer.add_scalar(
                            "disc_score/gradient_penalty",
                            grad_pen.mean().data.item(),
                            iteration,
                        )
            ...
            if iteration % opt.gen_freq == 0:
                ...
                if opt.generator == "DSGAN":
                    ...
                    #g_loss = g_loss_module(fake_tex.detach(), fake_img.detach(), bicubic_img)
                    g_loss = g_loss_module(fake_tex.detach(), fake_img, bicubic_img)
                else:
                    raise NotImplementedError(
                        "Discriminator model [{:s}] not recognized".format(opt.D)
                    )
                ...
                if opt.saving:
                    writer.add_scalar(
                        "loss/perceptual_loss", g_loss_module.last_per_loss, iteration
                    )
                    writer.add_scalar(
                        "loss/color_loss", g_loss_module.last_col_loss, iteration
                    )
                    writer.add_scalar(
                        "loss/g_tex_loss", g_loss_module.last_tex_loss, iteration
                    )
            ...
            if opt.saving:
                ...
                writer.add_scalar(
                    "disc_score/real", real_tex.mean().data.item(), iteration
                )
                writer.add_scalar(
                    "disc_score/fake", fake_tex.mean().data.item(), iteration
                )
        ...
        if opt.saving:
            writer.add_scalar(
                "param/learning_rate", torch.Tensor(scheduler_g.get_lr()), epoch
            )
        ...
        if epoch % opt.val_interval == 0 or epoch % opt.val_img_interval == 0:
            ...
            with torch.no_grad():
                ...
                for index, (hr_img, bicubic_img, disc_img, target_img) in enumerate(
                    val_bar
                ):
                    if opt.generator == "DSGAN":
                        input_img = bicubic_img
                    ...
                    print('Use Perceptual loss: {0}'.format(str(opt.use_per_loss)))
                    if opt.use_per_loss == False:
                        per_loss_sum = 0
                    else:
                ...
                if opt.saving and len(val_loader) > 0:
                    ...
                    writer.add_scalar(
                        "val/rgb_error", rgb_loss_sum / len(val_set), iteration
                    )
                    writer.add_scalar(
                        "val/mean_error", mean_loss_sum / len(val_set), iteration
                    )
                    writer.add_scalar(
                        "val/perceptual_error", per_loss_sum / len(val_set), iteration
                    )
                    writer.add_scalar(
                        "val/color_error", col_loss_sum / len(val_set), iteration
                    )
                    ...
                    if epoch % opt.val_img_interval == 0 and epoch != 0:
                        ...
                        val_images = torch.chunk(
                            val_images, val_images.size(0) // (n_val_images * 5)
                        )
                        val_save_bar = tqdm(val_images, desc="[Saving results]")
                        for index, image in enumerate(val_save_bar):
                            image = tvutils.make_grid(
                                image, nrow=n_val_images, padding=5
                            )
                            out_path = "val/target_fake_tex_disc_f-wav_t-wav_" + str(
                                index
                            )
                            writer.add_image(
                                "val/target_fake_crop_low_high_" + str(index),
                                image,
                                iteration,
                            )
        ...
        if opt.saving and epoch % opt.save_model_interval == 0 and epoch != 0:
            path = os.path.join(
                save_path, "checkpoints", "iteration_{}.tar".format(iteration)
            )
