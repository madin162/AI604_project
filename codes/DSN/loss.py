...
def generator_loss(labels, wasserstein=False, weights=None):
    ...
    for label, weight in zip(labels, weights):
        if wasserstein:
            loss = loss + weight * torch.mean(-label)
        else:
            loss = loss + weight * torch.mean(-torch.log(label + 1e-8))
    return loss


def discriminator_loss(
    reals, fakes, wasserstein=False, grad_penalties=None, weights=None
):
    ...
    if wasserstein:
        ...
        for real, fake, weight, grad_penalty in zip(
            reals, fakes, weights, grad_penalties
        ):
            loss = loss + weight * (-real.mean() + fake.mean() + grad_penalty)
    else:
        for real, fake, weight in zip(reals, fakes, weights):
            loss = loss + weight * (
                -torch.log(real + 1e-8).mean() -
                torch.log(1 - fake + 1e-8).mean()
            )
    return loss
...
class GeneratorLoss(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if self.per_type == "LPIPS":  # Default
            self.perceptual_loss = PerceptualLoss(
                rotations=lpips_rot_flip, flips=lpips_rot_flip
            )
        ...
        self.use_per_loss = use_per_loss
    ...
    def forward(self, tex_labels, out_images, target_images):
        ...
        self.last_tex_loss = generator_loss(
            tex_labels, wasserstein=self.wasserstein)
        # Perception Loss
        if self.use_per_loss:
    ...
    def mean_loss(self, x, y):
        return self.pixel_loss(
            x.view(x.size(0), -1).mean(1), y.view(y.size(0), -1).mean(1)
        )
...
class PerceptualLossLPIPS(nn.Module):
    def __init__(self):
        ...
        self.loss_network = ps.PerceptualLoss(
