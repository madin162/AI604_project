...
class Generator(nn.Module):
    def __init__(self, n_res_blocks=8):
        ...
        self.block_input = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, padding=1), nn.PReLU()
        )
...
class Discriminator(nn.Module):
    def __init__(
        ...
        if D_arch.lower() == "nld_s1":
            ...
            print(
                "# Initializing NLayer-Discriminator-stride-1 with {} norm-layer".format(
                    norm_layer.upper()
                )
            )
            ...
            print(
                "#Initializing NLayer-Discriminator-stride-2 with {} norm-layer".format(
                    norm_layer.upper()
                )
            )
            ...
            print(
                "# Initializing FSSR-DiscriminatorBasic with {} norm layer".format(
                    norm_layer
                )
            )
        else:
            raise NotImplementedError(
                "Discriminator architecture [{:s}] not recognized".format(
                    D_arch)
            )
    ...
    def forward(self, x, y=None):
        ...
        x = self.net(x)
        if y is not None:
            x = x - self.net(self.filter(y)).mean(0, keepdim=True)
        if not self.wgan:
            x = torch.sigmoid(x)
        return x
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        if norm_layer.lower() == "instance":
            ...
            raise NotImplementedError(
                "Norm layer [{:s}] not recognized".format(norm_layer)
            )
        if (
...
if __name__ == "__main__":
    model_g = Generator()
    t = torch.load(
        "/media/4T/Dizzy/github/DASR/DSN_experiments/0121_DeResnet+avgpool+FSD+wtex0.03/checkpoints/last_iteration.tar"
    )
