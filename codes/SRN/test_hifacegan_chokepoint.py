...
def __main__():
    ...
    for test_loader in test_loaders:
        ...
        dataset_dir = os.path.join(
            opt['path']['results_root'], test_set_name, 'imgs')
        util.mkdir(dataset_dir)
        if opt['save_RealorFake']:
            dataset_dir_patchrealfake = os.path.join(
                opt['path']['results_root'], test_set_name, 'RealOrFake')
        ...
        for data in test_loader:
            ...
            data['LR'] = transforms.Resize(
                [data['LR'].shape[2]*4, data['LR'].shape[3]*4])(data['LR'])
            ...
            if suffix:
                save_img_path = os.path.join(
                    dataset_dir, img_name + suffix + '.png')
            else:
                save_img_path = os.path.join(dataset_dir, img_name + '.png')
            util.save_img(sr_img, save_img_path)

            if opt['save_RealorFake']:
                save_patch_path = os.path.join(
                    dataset_dir_patchrealfake, img_name + '.npy')
            ...
            if need_HR:
                ...
                #logger.info('{} - mean_rf: {}'.format(img_name,
                #            np.mean(visuals['realorfake'])))
        ...
        if need_HR:  # metrics
            ...
            if opt['val_lpips']:
                avg_lpips = sum(test_results['lpips']) / \
                    len(test_results['lpips'])
