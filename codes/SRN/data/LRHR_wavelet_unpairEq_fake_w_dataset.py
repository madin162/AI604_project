...
class LRHR_wavelet_Equnpair_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_unpair_HR_temp = np.ascontiguousarray(np.transpose(img_unpair_HR, (2, 0, 1)))
        img_LR_real_temp = np.ascontiguousarray(np.transpose(img_LR_real, (2, 0, 1)))
        img_LR_fake_temp = np.ascontiguousarray(np.transpose(img_LR_fake, (2, 0, 1)))

        img_HR = np.zeros((np.shape(img_HR_temp)[0],HR_size,HR_size))
        img_HR[:,:np.shape(img_HR_temp)[1],:np.shape(img_HR_temp)[2]]=img_HR_temp
        img_unpair_HR = np.zeros((np.shape(img_unpair_HR_temp)[0],HR_size,HR_size))
        img_unpair_HR[:,:np.shape(img_unpair_HR_temp)[1],:np.shape(img_unpair_HR_temp)[2]]=img_unpair_HR_temp
