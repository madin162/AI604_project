...
class alexnet_trunc(torch.nn.Module):
    def __init__(self, requires_grad=False, pretrained=True):
        super(alexnet_trunc, self).__init__()
        alexnet_pretrained_features = tv.alexnet(
            pretrained=pretrained).features
        self.slice1 = torch.nn.Sequential()
        self.slice2 = torch.nn.Sequential()
        ...
        self.slice5 = torch.nn.Sequential()
        self.N_slices = 5
        self.slice1.add_module(str(0), alexnet_pretrained_features[0])
        self.slice2.add_module(str(1), alexnet_pretrained_features[1])
        for x in range(5, 8):
            self.slice3.add_module(str(x), alexnet_pretrained_features[x])
        for x in range(8, 10):
            self.slice4.add_module(str(x), alexnet_pretrained_features[x])
    ...
    def forward(self, X):
        ...
        h_relu1 = h
        alexnet_outputs = namedtuple(
            "AlexnetOutputs", ['relu1'])
        out = alexnet_outputs(h_relu1)
